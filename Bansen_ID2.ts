    ...
    for (let i in SheetData) {
      ...
      if (SheetData[i][32] == errortext) {
        ...
        WorkData = await RPA.Google.Spreadsheet.getValues({
          spreadsheetId: `${SSID}`,
          range: `${SSName}!A${Row}:AC${Row}`
        });
        await RPA.Logger.info(WorkData);
        // 作業用フラグ(AG列)に"作業中"と記載
...
async function ClusterJudgeError() {
  ...
  await SheetJudgeError(Row);
  // テスト
  await RPA.Logger.info('キャンセルします');
  await RPA.WebBrowser.driver.navigate().back();
  await RPA.sleep(5000);
}
...
async function GetCampaignId() {
  ...
  // キャンペーン・広告貼り付けシートのBN列に発番したキャンペーンIDを記載
  await RPA.Google.Spreadsheet.setValues({
    spreadsheetId: `${SSID}`,
...
async function AdJudgeError() {
  ...
  if (
    NotAdvertisementSlotIdText == '該当する放送枠が存在しません' ||
    NotAdvertisementSlotIdText == '既に放送終了している放送枠です'
  ) {
    await RPA.Logger.info(
